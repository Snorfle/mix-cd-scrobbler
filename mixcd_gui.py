...
class MixCDGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Button(button_frame, text="Delete CD", command=self.delete_cd_dialog).pack(side=tk.LEFT, padx=(0, 10))
    ...
    def refresh_cd_list(self):
        """Refresh the CD dropdown list - now sorted alphabetically"""
        cd_items = []
        
        # Sort CDs alphabetically by title
        sorted_cds = sorted(self.cd_db.cds.items(), key=lambda x: x[1]['title'].lower())
        
        for cd_id, cd_info in sorted_cds:
    ...
    def get_selected_cd_info(self):
        """Get the currently selected CD info - works with alphabetical sorting"""
        selection = self.selected_cd.get()
        if not selection:
            return None, None
        
        # Get sorted CD list to match the dropdown order
        sorted_cds = sorted(self.cd_db.cds.items(), key=lambda x: x[1]['title'].lower())
        
        # Extract CD index from the dropdown selection
        cd_index = self.cd_combo.current()
        if cd_index >= 0 and cd_index < len(sorted_cds):
            cd_id, cd_info = sorted_cds[cd_index]
    ...
    def delete_cd_dialog(self):
        """Show delete CD dialog"""
        cd_id, cd_info = self.get_selected_cd_info()
        if not cd_info:
            messagebox.showerror("No CD Selected", "Please select a CD to delete")
            return
        ...
        if result:
            if self.cd_db.delete_cd(cd_id):
                print(f"✓ Deleted '{cd_info['title']}'")
                self.refresh_cd_list()
            else:
                messagebox.showerror("Delete Failed", "Could not delete the CD")
