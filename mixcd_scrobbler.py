...
class MixCDDatabase:
    ...
    def load_database(self):
        ...
        try:
            if os.path.exists(self.db_file):
                ...
                self.cds = {}
                print("✓ Started with empty mix CD database")
        except Exception as e:
    ...
    def save_database(self):
        ...
        try:
            print(f"Saving database to: {os.path.abspath(self.db_file)}")
            with open(self.db_file, 'w', encoding='utf-8') as f:
                json.dump(self.cds, f, indent=2, ensure_ascii=False)
            print(f"✓ Database saved with {len(self.cds)} CDs")
        except Exception as e:
            print(f"Error saving database: {e}")
    
    def list_cds(self):
        """List all mix CDs in database - sorted alphabetically"""
        if not self.cds:
        ...
        sorted_cds = sorted(self.cds.items(), key=lambda x: x[1]['title'].lower())
        
        for i, (cd_id, cd_info) in enumerate(sorted_cds, 1):
    ...
    def select_cd(self):
        """Interactive CD selection - sorted alphabetically"""
        if not self.cds:
        ...
        try:
            ...
            # Get sorted CD list to match the displayed order
            sorted_cds = sorted(self.cds.items(), key=lambda x: x[1]['title'].lower())
            
            if 0 <= choice < len(sorted_cds):
                cd_id, cd_info = sorted_cds[choice]
